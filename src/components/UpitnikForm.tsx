...
export default function UpitnikForm() {
	...
	const sendFormData = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			const response = await fetch('https://formcarry.com/s/mkgw3-pbDOJ', {
				method: 'POST',
				headers: {
			...
			if (!response.ok) {
				...
				setMessage('Greška prilikom slanja upita. Pokušajte ponovo kasnije.')
			} else {
				setMessageType('success')
				setMessage('Upit je uspješno poslan!')
	...
	return (
		<form onSubmit={sendFormData} className="space-y-8">
			...
			<div>
				...
				<label className="mb-2 block">Koja je osnovna svrha web stranice?</label>
				<input
					name="svrhaStranice"
				...
				<label className="mb-2 mt-4 block">Imate li postojeću web stranicu?</label>
				<select
					name="hasWebsite"
				...
				{form.hasWebsite === 'Da' && (
					<div className="mt-4">
						<label className="mb-2 block">Upišite URL postojeće web stranice</label>
						<input
							name="websiteDomain"
				...
				<label className="mb-2 mt-4 block">Imate li primjer web stranice koja vam se sviđa?</label>
				<input
				...
				<label className="mb-2 mt-4 block">Treba li stranica biti višejezična?</label>
				<select
					name="isMultilingual"
			...
			<div>
				...
				/>
				<label className="mb-2 mt-4 block">Želite da vam mi izradimo dizajn?</label>
				<p className="text-sm">
					Možemo vam izraditi dizajn po uzoru na stranicu koja vam se sviđa, a tokom izrade možete
					predlagati izmjene.
				</p>
				<select
					name="zelimVasDizajn"
			...
			{/* CMS I SADRŽAJ */}
			<div>
				<h2 className="mb-2 text-xl font-semibold">Upravljanje sadržajem (CMS)</h2>
				...
				{form.wantsCMS === 'Da' && (
					<div className="mt-4">
						<label className="mb-2 mt-4 block">Koje sadržaje želite sami uređivati?</label>
						<textarea
					...
					<>
						<div className="mt-4">
							<label className="mb-2 mt-4 block">
								Želite li da se plaćanje vrši na web stranici ili da vam stigne upit na mail na koji
								vi šaljete ponudu?
							</label>
							<textarea
								name="payment"
						...
						<p>
							Ako je plaćanje direktno na web stranici, najčešće je potrebno imati upravljanje
							zalihama
						</p>
						<label className="mb-2 mt-4 block">Treba li upravljanje zalihama? </label>
						<select
							name="needsInventory"
						...
						<label className="mb-2 mt-4 block">Želite li imati akcije, popuste, kupone?</label>
						<textarea
			...
			<div>
				...
				<label className="mb-2 block">Koji buđet ste predvidjeli za izradu stranice?</label>
				<p className="text-sm">
					Ova informacija nam je važna kako bismo mogli predložiti rješenje koje realno odgovara
				...
				<input
					...
					type="text"
					className="w-full rounded border p-2"
					placeholder="npr. u roku 2 tjedna dana, 2 mjeseca..."
				/>
			</div>
			...
			<div className="text-center">
				<button
					type="submit"
					disabled={loading}
					className="rounded bg-blue-600 px-6 py-3 text-white transition hover:bg-blue-700 disabled:bg-gray-400"
				>
